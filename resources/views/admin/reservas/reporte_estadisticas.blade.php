...
@php $criti=[];@endphp
  ...
  <script type="text/javascript">
    function cambioDeFecha(id_evento) {
      window.location = "/admin/reporte_estadisticas/" + id_evento
    }
    ...
    $(document).ready(function(){
      ...
      @if ($total_hombres_cortesia || $total_mujeres_cortesia)
        var options = {
          exportEnabled: false,
          animationEnabled: true,
          title:{
            text: "Cortesías entregadas: {{$total_cortesias}}"
          },
          data: [{
            type: "pie",
            showInLegend: true,
            ...
            indexLabelPlacement: "inside",
            dataPoints: [
              { y: {{$total_hombres_cortesia}}, name: "{{$total_hombres_cortesia}} Hombres", indexLabelFontSize: 20 },
              { y: {{$total_mujeres_cortesia}}, name: "{{$total_mujeres_cortesia}} Mujeres", indexLabelFontSize: 20 },
            ]
          }]
        };
        $("#grafica-pendientes").CanvasJSChart(options);
