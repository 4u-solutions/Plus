          ...
          <div class="row">
            <div class="col-12 border shadow p-1 text-end">
              ...
              <div class="row {{$cobrar ? 'd-block' : 'd-none'}}" id="boton-cobrar">
                  <a href="{{route('aprobar_cortesia', ['id_pedido' => $id_pedido])}}" class="btn btn-dark w-100 m-auto d-block fs-1"><i style="height: 1.8rem; width: 1.8rem;" data-feather="check"></i> DESPACHAR </a>
                </form>
              </div>
            </div>
          </div>
  ...
  <script type="text/javascript">
    ...
    function agregarBotellas(id_pedido, id_producto) {
      Swal.fire({
        ...
        title: `<div class="modal-header" style="padding: 0; margin: auto; border:none;">
                  <h1 class="modal-title" id="verifyModalContent_title">AGREGAR CORTESÍA</h1>
              </div>`,
        html:`
        ...
        if (result.isConfirmed) {
          ...
          var ruta     = "/admin/cargar_cortesia/" + id_pedido + "/" + id_producto + "/" + cantidad
          $.ajax({
              type: "GET",
              url: ruta,
              dataType: "JSON",
              success: function(respuesta){
                ...
                var ruta = "/admin/aprobar_cortesia/" + respuesta.id_pedido + '/' + encodeURI(nombreCliente)
    ...
    function borrarBotella(id_detalle, subtotal) {
      Swal.fire({
        ...
        title: `<div class="modal-header" style="padding: 0; margin: auto; border:none;">
                  <h1 class="modal-title" id="verifyModalContent_title">BORRAR CORTESÍA</h1>
              </div>`,
        text: '¿Está seguro de borrar la cortesía?'
      }).then(result => {
        if (result.isConfirmed) {
